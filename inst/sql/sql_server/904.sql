...
) A on A.person_id = P.person_id  AND A.START_DATE >= DATEADD(day,-183,P.START_DATE) AND A.START_DATE <= DATEADD(day,0,P.START_DATE) ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
GROUP BY p.person_id, p.event_id
HAVING COUNT(cc.event_id) = 0
-- End Correlated Criteria
...
(
  ...
  (
    ...
    (
      --cteEndDates
      select PERSON_ID, DATEADD(day,-1 * 183,EVENT_DATE) as END_DATE -- unpad the end date by 183
      FROM
      (
				...
				(
					...
					-- add the end dates with NULL as the row number, padding the end dates by 183 to allow a grace period for overlapping ranges.
					select PERSON_ID, DATEADD(day,183,DRUG_EXPOSURE_END_DATE), 1 as EVENT_TYPE, NULL
					FROM #drugTarget D
				) RAWDATA
      ) E
